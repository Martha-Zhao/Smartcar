...
int main()
{
    ...
    int count_zeros[92][16] = {0};  //0的个数 和 0所在的位置

    int *count_zeros_p;
    int *loc_p;
    int *ans_p;

    int loc_changed [][8] = {0}; //去掉0后的棋子位置数组
    int ans_changed [][8] = {0}; //去掉0后的解的数组
...
//                    count_zeros[i][&(count_zeros[i][0])+1] = k;
    ...
    printf("sizeof loc_changed is %d\n",sizeof(loc_changed[0])/sizeof(int));
